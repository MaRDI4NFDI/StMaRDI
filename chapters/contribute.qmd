...
To upload a dataset or dataset collection, \[Step 1\] click on the '+' to create a new record and \[Step 2\] select the [Graphical Modeling and Causal Inference](https://zenodo.org/communities/mardigmci) community. \[Step 3\] Upload the desired files and give as much additional information as possible in the cells below.
...
For good reference, see the [ALARM (A Logical Alarm Reduction Mechanism)](https://zenodo.org/records/14793281) dataset and the [Sachs: Protein and Phospholipids Expressions](https://zenodo.org/records/7681811) dataset collection.
::::

::: {.callout-note collapse="true" title="Analysis Notebook" icon="false"}
Prepare a git repository from the [GMCI notebook template](https://github.com/MaRDI4NFDI/GMCI_notebook_template). Follow the [Analysis Notebook Template](notebook_gallery/notebooks/GMCI_notebook_template/notebook.qmd) (`notebook.qmd`) and set the picture for the library as `gallery_picture.png`.

Log into [Zenodo](https://zenodo.org/) via GitHub, ORCID, OpenAIRE, or create a new account. Then, [connected your repository with Zenodo](https://zenodo.org/account/settings/github/) as shown in the picture below.

![Source: <https://zenodo.org/account/settings/github/>](../images/zenodo_git_integration.png)

This will automatically create a record of the zipped repository along with a unique DOI. As the whole repository is zipped, we advise to restrict the repository's content to the necessary files. The associated DOI will forward you to the Zenodo record, which you can submit it to the [Graphical Modeling and Causal Inference](https://zenodo.org/communities/mardigmci/records?q=&l=list&p=1&s=10) community by editing the record's page. Although the record's description is taken from the git release, the records meta-information can be edited manually afterwards, e.g., referencing a publication, linking the associated Zenodo dataset or specifying an R package.

We connect the accepted repository with `git submodule add`, render `notebook.qmd` and showcase the analysis on this website's [library](/chapters/notebooks.qmd).
...
For good reference, see the [PC Algorithm for Causal Discovery](https://github.com/lmareis/PC-Algorithm-An-Application-on-the-Alarm-Dataset).
:::

For both options, a [Zenodo](https://zenodo.org/) record has to be created according to the structure above and submitted to the [GMCI Zenodo community](https://zenodo.org/communities/mardigmci/records?q=&l=list&p=1&s=10). The [community moderators](/index.qmd#the-initiative) process each submission, create the necessary embedding links, potentially request revisions, and assist with any questions. While accepted datasets appear in the GMCI Zenodo community, notebook submissions' git repositories will be backed on Zenodo with the analysis rendered and featured on this website's [library](/chapters/notebooks.qmd).
